...
| Queue | `features/queue/job-queue-table.tsx`, `job-status.ts` | Job table, FIFO, row click → details, status labels (S25), action slots Review/Pause/Resume → Restart → Cancel; Open output only on `COMPLETED` (S26); pagination (10/25/50/All) + sticky header + 10-row scroll viewport |
| Prescan | `features/prescan/prescan-review-modal.tsx` | Review modal: canvas, OCR, scrubber; Cancel + Confirm; `render_video` toggle (6.11) |
...
| Button | Review, Pause, Resume, Restart, Cancel; Open output on completed |
| Select | Task type |
