...
class TrafficConfig:
    ...
    # Restored Calibrated Coordinates
...
class Visualizer:
    ...
    def __init__(self, config: TrafficConfig, width: int, height: int):
        ...
        # [FIX] Cache explicit coordinates as standard tuples for robust cv2 rendering
        self.line_pt1 = (int(config.counting_start[0] * width), int(config.counting_start[1] * height))
        self.line_pt2 = (int(config.counting_end[0] * width), int(config.counting_end[1] * height))
        
        # Initialize LineZone for trigger logic, decoupling from rendering operations
        line_start_point = sv.Point(x=self.line_pt1[0], y=self.line_pt1[1])
        line_end_point = sv.Point(x=self.line_pt2[0], y=self.line_pt2[1])
    ...
    def draw(self, frame: np.ndarray, dets: sv.Detections, debug_info: dict, counts_in: dict, counts_out: dict) -> np.ndarray:
        ...
        # [FIX] Utilize explicit cached coordinate tuples to prevent API breaking changes
        cv2.line(frame, self.line_pt1, self.line_pt2, (0, 255, 0), 2)
    ...
    def run(self) -> None:
        ...
        while self.stream.cap.isOpened():
            ...
            if len(dets) > 0:
                ...
                filtered_dets = dets[centers_y > cutoff]
            else:
                filtered_dets = dets
                
            dets = self.tracker.update(filtered_dets)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_b", default="/app/ViAna/models/pretrained/yolo11l.pt")
