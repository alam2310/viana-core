...
# 0:Car, 1:Jeep, 2:Van, 3:MiniBus, 4:MTW, 5:Auto, 6:CityBus, 7:Truck, 8:LCV, 9:Cycle, 10:Others
...
def run_ensemble(video_path, model_a_path, model_b_path, output_path):
    ...
    model_b = YOLO(model_b_path) # Auto-downloads yolo11m.pt
    model_b.to(DEVICE_B)
    ...
    while cap.isOpened():
        ...
        # Explicitly enforce device='cuda:0'
        results_a = model_a.predict(frame, imgsz=1088, conf=0.25, device=DEVICE_A, verbose=False)[0]
        
        if results_a.boxes:
            for box in results_a.boxes:
                # .cpu().numpy() creates a COPY in System RAM.
                # This makes it safe to mix with data from GPU 1 later.
        ...
        if results_b.boxes:
            for box in results_b.boxes:
                ...
                # Logic happens on CPU lists, so no GPU conflict exists here
    ...
    cap.release()
    out.release()
    print(f"\n✅ Dual-GPU Inference Complete. Output: {output_path}")
...
if __name__ == "__main__":
    ...
    # [UPGRADE] Default changed to yolo11m.pt (Medium)
    parser.add_argument("--model_b", type=str, default="yolo11m.pt", help="Medium Person Model")
    parser.add_argument("--out", type=str, default="debug_dual_gpu.mp4", help="Output path")
